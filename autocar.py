...
class AutoCar(vehicle.Vehicle):
    """This class take all the previous classes and create a auto car model, it can
    see the map over the camera and move based on it

    To Inicialize a self driving car it is need:
    ...
    * The length of the camera in pixels (**cam_length**)"""
    ...
    def __init__(self, map_file, cam_length=128, L=0.2, alpha_max=20,
        ...
        self.cam.loadMap(self.map)  # the vehicle camera
    ...
    def show(self, legend="Path"):
        """Show the image of the map and the poses of the auto car"""
        self.map.show()
    ...
    def sim_LineFollower(self, Kp=2, Ki=0, Kd=0, v=5, Steps=500, debug=False):
        """Simulate the auto car autonomously running througth the map using a PID
